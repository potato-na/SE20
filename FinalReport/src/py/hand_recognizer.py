...
from pose import PoseRecognizer, Pose
from hand_gesture import HandGestureRecognizer
from encoder import HandDataEncoder
from websocket_py.client import Client

camera_ip = '192.168.11.57'
camera_port = '8080'
HOST_ADDR = '192.168.11.57'
HOST_PORT = '9001'
...
def get_args():
	...
	parser.add_argument('--use_stream_camera', action='store_true')
...
def main():
	...
	use_stream_camera = args.use_stream_camera

	# カメラ
	if use_stream_camera:
		camera_url = f'http://{camera_ip}:{camera_port}/?action=stream'
	...
	# ハンドポーズ, ジェスチャー
	pose_recognizer = PoseRecognizer(frame=10)
	gesture_recognizer = HandGestureRecognizer()
	encoder = HandDataEncoder()
	current_pose = Pose.NONE
	previous_pose = Pose.NONE
	previous_pinch_y = None # 前回のピンチポーズ時のy座標
	gesture = (False, None)

	# websocketクラインとの起動
	url = f'ws://{HOST_ADDR}:{HOST_PORT}/'
	client = Client(url, print_on_message=False)
	...
	while True:
		...
		if results.multi_hand_landmarks is not None:
			for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
				...
				# ポーズの推定
				current_pose = pose_recognizer.ranking_handpose(hand_landmarks.landmark)
				# ジェスチャーの推定
				parm_height = abs(hand_landmarks.landmark[12].y - hand_landmarks.landmark[0].y)
				gesture = gesture_recognizer.is_swing(current_x=hand_landmarks.landmark[9].x, parm_height=parm_height)
				
				# 座標データ送信用
				output_hand_landmarks = hand_landmarks.landmark
		...
		# ハンドポーズが five の時のみジェスチャーを検知
		if current_pose == Pose.FIVE:
			if gesture[0]:
				send_data = encoder.encode("Gesture", gesture[1])
				print(gesture[1])
				if use_stream_camera:
		...
		if current_pose != previous_pose:
			# ポーズ情報の送信
			send_data = encoder.encode(str(current_pose))
			print(send_data)
			if use_stream_camera:
				client.send(send_data)
		...
		# ピンチポーズ
		elif current_pose == Pose.PINCH:
			# ピンチポーズの時はy座標と共に送信
			pinch_y = '{:.2f}'.format(-output_hand_landmarks[9].y)
			if pinch_y != previous_pinch_y:
				send_data = encoder.encode(str(current_pose), pinch_y)
				print(send_data)
				if use_stream_camera:
					client.send(send_data)
				previous_pinch_y = pinch_y


		cv.putText(debug_image, "Pose:" + str(current_pose), (10, 30),
